...
async function submitForm() {
  ...
  try {
    ...
    if (data.success) {
      ...
      document.querySelectorAll('.step-dot').forEach(d => { d.classList.remove('active'); d.classList.add('done'); d.innerHTML = '<span>✓</span>'; });
      document.getElementById('progressFill').style.width = '100%';
    }
  } catch {
    showToast('Something went wrong. Please try again.', 'error');
    submitBtn.disabled = false;
    submitBtn.innerHTML = '<span class="btn-submit-icon">🚀</span> Submit';
  }
}
